...
-- Corrige a referência ambígua a `tentativa` dentro de uma função que também
-- possui coluna de saída homônima. Mantém a lógica da 0031 e qualifica a
-- coluna da tabela explicitamente.
  ...
  if found then
    ...
    if v_auditoria.estado = 'reservada' then
      if v_auditoria.reservado_em < now() - interval '2 minutes' then
        ...
        where a.id = v_auditoria.id returning a.* into v_auditoria;
    ...
    if v_auditoria.estado = 'falhou' and v_auditoria.tentativa < 3 then
      update auditoria.execucoes_ia as a
      set estado = 'reservada', reservado_em = now(), tentativa = a.tentativa + 1,
